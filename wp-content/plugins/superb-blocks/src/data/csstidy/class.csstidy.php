...
class csstidy
{
	...
	/**
	 * Write plain output to a file // Updated for WP
	...
	public function write($filename, $formatted = false, $doctype = 'xhtml1.1', $externalcss = true, $title = '', $lang = 'en', $pre_code = true)
	{
		...
		$temp_dir = trailingslashit($upload_dir['path'] . '/csstidy-temp');
		...
		// Write file contents
		if (!$formatted) {
			if (!$wp_filesystem->put_contents($filepath, $this->print->plain(), FS_CHMOD_FILE)) {
